...
bash $DOTPATH/lib/dir-hash.sh $name $archive_dir/$name.tar.gz $next_last_num
...
$DOTPATH/bin/aws s3 cp $archive_dir/$name.tar.gz.$next_last_num s3://$s3_archive_bucket/$s3_archive_prefix/$name.tar.gz.$next_last_num
